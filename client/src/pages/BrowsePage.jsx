...
function ListingCard({ listing }) {
  ...
  return (
    <div
      ...
      style={{ transform: hovered ? "translateY(-6px)" : "translateY(0)", transition: "all 0.3s cubic-bezier(0.34,1.56,0.64,1)" }}
      className="bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-2xl cursor-pointer group"
    >
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          ...
          <a
            href={listing.contact?.whatsapp
              ? `https://wa.me/${listing.contact.phone.replace(/\D/g, "")}`
              : `tel:${listing.contact?.phone}`}
            onClick={(e) => e.stopPropagation()}
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-white text-sm font-semibold shadow transition-all hover:scale-105 active:scale-95"
            style={{ backgroundColor: color }}
          >
            {listing.contact?.whatsapp ? "💬" : "📞"} Contacter
          </a>
        </div>
      </div>
    </div>
...
export default function BrowsePage() {
  ...
  const [activeMaterial, setActiveMaterial] = useState("Tous");
  ...
  const filtered = listings
    .filter((l) => !search || l.materialType.toLowerCase().includes(search.toLowerCase()) || l.location?.city?.toLowerCase().includes(search.toLowerCase()) || l.description?.toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(160deg, #0a2e1a 0%, #1B4332 40%, #2d6a4f 100%)" }}>
